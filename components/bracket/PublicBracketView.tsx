...
export default function PublicBracketView({ tournamentId, isLive }: Props) {
  ...
  return (
    <div className="space-y-10">
      {champion && (
        <div className="flex items-center justify-center gap-2 rounded-md bg-brand-50 px-4 py-3 text-brand-700">
      ...
      <div>
        <h2 className="mb-3 text-lg font-semibold text-gray-900">Completed Matches</h2>
        {completedMatches.length === 0 ? (
          <p className="text-sm text-gray-500">No matches completed yet.</p>
        ) : (
          <ul className="space-y-2 text-sm">
            {completedMatches.map((match) => (
              ...
              >
                <span className="text-gray-500">{match.roundName}</span>
